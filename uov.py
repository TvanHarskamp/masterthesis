...
def integer_remainder(x, modulus):
    return torch.div(torch.mul(x, math.pi))
...
class VerificationLayer(nn.Module):
    ...
    def forward(self, m, s):
        ...
        m_check = torch.remainder(torch.cat([s_T @ x @ s for x in self.P]) - m, 2)
        ...
        return nn.functional.relu(torch.add(torch.mul(torch.sum(m_check), -1), 1), inplace=True)
